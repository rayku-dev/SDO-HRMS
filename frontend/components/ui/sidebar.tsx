...
>(({ defaultOpen = true, open: openProp, onOpenChange: setOpenProp, className, style, children, ...props }, ref) => {
  ...
  const open = openProp ?? _openMobile
  ...
  const contextValue = React.useMemo<SidebarContext>(
    ...
    [state, open, setOpen, _openMobile, isMobile, toggleSidebar]
  )
...
>(({ variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={cn(
        "group peer hidden md:block text-sidebar-foreground sticky top-0 h-svh",
    ...
    >
      <div
        className={cn(
          "flex h-full w-full flex-col bg-sidebar transition-[width,height,padding] duration-200 ease-linear",
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col gap-1 justify-center">
        <div className="h-2 w-20 rounded-sm bg-sidebar-primary/10" />
      </div>
    </div>
  )
})
